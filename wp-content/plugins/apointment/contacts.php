...
***********************************************
*/
add_action('wp_ajax_health_contacts', 'health_contacts');
add_action('wp_ajax_nopriv_health_contacts', 'health_contacts');

function health_contacts(){
	global $wpdb;
	$table = $wpdb->prefix . "contacts"; 
	//ob_start();
	$name    = esc_attr(strip_tags($_POST["form_name"]));
    $email   = esc_attr(strip_tags($_POST["form_email"]));
    $subject = esc_attr(strip_tags($_POST["form_subject"]));
    $phone 	 = esc_attr(strip_tags($_POST["form_phone"]));
    $message = esc_attr(strip_tags($_POST["form_message"]));

    if ( $_POST["form_name"] != ""  && $_POST["form_email"] != ""  && $_POST["form_subject"] != ""  && $_POST["form_phone"] != ""  &&  $_POST["form_message"] != "") {
	if($wpdb->insert( $table ,array( 
		...
		echo "<p style='color:green;font-size:14px;'>Hi ".$name.",<br> Your Data successfully recorded !!</p>";
		wp_die();
	}
